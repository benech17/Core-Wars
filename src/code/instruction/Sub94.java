...
public class Sub94 extends Instruction {

	public Sub94(int a, int b, int c, int d,int e) {
		super(a, b, c, d, e);
	}
	...
	public boolean estValide() {
		if(getAdMode1()==0) {
			...
			throw new RedCodeException("SUB prend un premier argument");
		}
		if(getAdMode2()==0 || getAdMode2()==2) {
			System.out.println(this);
			throw new RedCodeException("SUB prend une adresses en 2 eme arguments");
		}
		if(getType()==7){
			System.out.println(this);
	...
	protected int fType(long[] memory, int read){
		...
		System.out.println("juste avant la fin"+this);
		memory[read%memory.length]=Traducteur94.traduireInstructionToLong(this);
		return read+1;
	}

...
public class Mod94 extends Instruction {

	public Mod94(int a, int b, int c, int d, int e) {
		super(a, b, c, d, e);
	}
	...
	public boolean estValide() {
		if(getAdMode1()==0) {
			...
			throw new RedCodeException("MOD prend un premier argument");
		}
		if(getAdMode2()==0 || getAdMode2()==2) {
			System.out.println(this);
			throw new RedCodeException("MOD prend une adresses en 2 eme arguments");
		}
		if(getType()==7){
			System.out.println(this);
	...
	protected int aType(long[] memory, int read){
		...
		switch(getAdMode1()){
			...
			if(!(instru instanceof Dat94)){return -1;}
			tmp=Traducteur94.traduireLongToInstruction(memory[(read+getArg1()+instru.getArg1())%memory.length]).getArg1();break;
		...
		if(tmp==0){System.out.println("Division par 0 interdite");return -1;}
		switch(getAdMode2()){
